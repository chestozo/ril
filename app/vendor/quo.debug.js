...
(function() {
  (function($$) {
    ...
    $$.fn.trigger = function(event, touch, originalEvent, firstTouch) {
  ...
  (function($$) {
    ...
    _onTouchEnd = function(event) {
      ...
      if (GESTURE.fingers === 1) {
        if (GESTURE.taps === 2 && GESTURE.gap) {
          ...
          if (swipe_direction) _trigger("swipe" + swipe_direction);
          _cleanGesture();
        } else {
          _trigger("tap");
          if (GESTURE.taps === 1) {
            TOUCH_TIMEOUT = setTimeout((function() {
              if (GESTURE.last && (Date.now() - GESTURE.last < 300)) { // try to fix hold to copy text feature
                _trigger("singleTap");
              }
        ...
        if (!anyevent && CURRENT_TOUCH[0]) {
          if (Math.abs(FIRST_TOUCH[0].x - CURRENT_TOUCH[0].x) > 10 || Math.abs(FIRST_TOUCH[0].y - CURRENT_TOUCH[0].y) > 10) {
            ...
            if (drag_direction) _trigger("drag" + drag_direction);
          }
        }
        _cleanGesture();
      }
    ...
    _trigger = function(type, params) {
      if (GESTURE.el) {
        ...
        return GESTURE.el.trigger(type, params, EVENT, FIRST_TOUCH && FIRST_TOUCH.length ? FIRST_TOUCH[0] : null);
      }
    };
    ...
    var getInfo = function(p1, p2) {
        ...
        function dir(a) {
            if (a > (360 - da) || a < da) { return 'Right'; }
            if (a > (180 - da) && a < (180 + da)) { return 'Left'; }
            if (a > (90 - da) && a < (90 + da)) { return 'Top'; }
            if (a > (270 - da) && a < (270 + da)) { return 'Bottom'; }
            return null;
        }
    ...
    _swipeDirection = function(x1, x2, y1, y2) {
      return getInfo([x1, y1], [x2, y2]).dir;
    };
